...
Subscribed topics:
  ...
  ~/scan_start    (geometry_msgs/Vector3)  x=half_width_deg, y=tilt_deg, z=steps
  ~/scan_abort    (std_msgs/Empty)         abort an in-progress sweep
...
class PanTiltNode(Node):
    ...
    def _scan_start_cb(self, msg: Vector3):
        """
        Vector3:
          x = half-width degrees (sweep runs from -x to +x)
          y = tilt degrees
          z = number of steps
        """
        if self._scan_thread and self._scan_thread.is_alive():
            self.get_logger().warn('scan already running; ignoring start')
            return
    ...
    def _publish_capture(self, index: int, total: int, pan_deg: float, tilt_deg: float):
        ...
        # encode step index/total in velocity slots for convenience
        cap.velocity = [float(index), float(total)]
        self._capture_pub.publish(cap)

    def _run_scan(self, half_width: float, tilt_deg: float, steps: int):
        """
        Executes a boustrophedon-free single-row sweep at fixed tilt:
          1. move to start (-half_width, tilt)        [big-move settle]
          2. step across to +half_width in `steps`     [per-step settle + dwell + capture]
          3. return home (0, 0)
        Runs in a daemon thread. Bus access is serialized by STSBus._lock.
        """
        try:
            self._status('started')

            # 1. move to scan start
            ...
            for i, pan in enumerate(positions):
                ...
                self._move(pan, tilt_deg, speed=self._scan_spd)

                # settle, then capture window
                if self._abort_evt.wait(self._scan_settle):
                    self._status('aborted')
                    self._move(0.0, 0.0)
                    return

                self._publish_capture(i, steps, pan, tilt_deg)
                self._status(f'step:{i}:{steps}')

                # dwell: hold still while assembler collects >= 1 full LD19 rotation
                if self._abort_evt.wait(self._scan_dwell):
                    self._status('aborted')
                    self._move(0.0, 0.0)
                    return
